...
def _read_1password_secret(uri: str) -> str:
...
def _build_op_uri(vault: str, item: str, section: Optional[str], field: str) -> str:
...
def fetch_credentials_from_1password(
    ...
    try:
        api_id = _read_1password_secret(_build_op_uri(vault, item, section, "api_id"))
        api_hash = _read_1password_secret(_build_op_uri(vault, item, section, "api_hash"))
        ...
        for cid in match_ids:
            try:
                aid = _read_1password_secret(_build_op_uri(vault, cid, section, "api_id"))
                ahash = _read_1password_secret(_build_op_uri(vault, cid, section, "api_hash"))
                if aid and ahash:
...
def _parse_message_ids(raw_str: str) -> Set[int]:
...
async def _resolve_chat_entity(client: TelegramClient, chat_arg: str):
...
def _build_filename(message: Message, chat_name: str) -> str:
    ...
    if message.file and message.file.name:
        ...
        stem = _sanitize_filename(Path(message.file.name).stem)[:40]
...
async def run_downloader(args: argparse.Namespace) -> None:
    ...
    if args.messages:
        target_msg_ids = _parse_message_ids(args.messages)
        print(f"[*] Filtering for {len(target_msg_ids)} specific message ID(s)")

    # Create output directory if necessary
    output_dir = Path(args.output_dir or f"./downloads/{_sanitize_filename(chat_title)}")
    ...
    async for msg in client.iter_messages(entity, **iter_kwargs):
        ...
        filename = _build_filename(msg, chat_title)
